  ...
  return (
    <div className="space-y-8 max-w-2xl">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="flex gap-3">
          {[
            ...
            return (
              <button key={opt.value} onClick={() => setTheme(opt.value)} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border transition-colors ${theme === opt.value ? 'border-[#E50914] bg-red-50 text-[#E50914]' : 'border-white/10 text-white/50 hover:border-white/15'}`}>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <select value={language} onChange={e => setLanguage(e.target.value)} className="w-full rounded-xl border border-white/10 px-4 py-2.5 text-sm focus:border-[#E50914] focus:outline-none">
          ...
          return (
            <div key={notif.label} className="flex items-center justify-between py-2">
              ...
              <button onClick={() => notif.set(!notif.value)} className={`relative h-6 w-11 rounded-full transition-colors ${notif.value ? 'bg-[#E50914]' : 'bg-gray-300'}`}>
                <span className={`inline-block h-4 w-4 rounded-full bg-white/5 transition-transform ${notif.value ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
          )
        })}

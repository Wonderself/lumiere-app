  ...
  if (!activeTemplate) {
    return (
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A2E] font-[family-name:var(--font-playfair)]">My Discussions</h1>
          <p className="text-sm text-gray-500 mt-1">8 templates stratégiques · Chat streaming avec agents experts</p>
        </div>
          ...
          {TEMPLATES.map(t => {
            ...
            return (
              <button
                ...
                className="text-left rounded-2xl border border-gray-200 bg-white p-6 hover:border-gray-300 hover:shadow-md transition-all"
              >
                ...
                <h3 className="text-sm font-bold text-[#1A1A2E] mb-1">{t.name}</h3>
                <p className="text-[10px] text-gray-500 mb-3">Agent: {t.agent}</p>
                <div className="space-y-1">
                  {t.starters.map(s => (
                    <p key={s} className="text-[10px] text-gray-500 truncate">• {s}</p>
                  ))}
                </div>
              </button>
            )
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)]">
      ...
      <div className="flex items-center gap-4 px-4 py-3 border-b border-gray-200">
        <button onClick={() => setActiveTemplate(null)} className="text-gray-500 hover:text-[#1A1A2E]">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <TIcon className="h-5 w-5" style={{ color: activeTemplate.color }} />
        <div>
          <p className="text-sm font-semibold text-[#1A1A2E]">{activeTemplate.name}</p>
          <p className="text-[10px] text-gray-500">{activeTemplate.agent}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mb-4">Commencez la discussion</p>
            <div className="flex flex-wrap gap-2 justify-center">
              {activeTemplate.starters.map(s => (
                <button key={s} onClick={() => sendMessage(s)} className="text-xs px-3 py-1.5 rounded-full border border-gray-200 text-gray-600 hover:bg-gray-50">{s}</button>
              ))}
            </div>
          </div>
        )}
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'user' ? 'bg-[#E50914] text-white' : 'bg-gray-100 text-gray-800'}`}>
      ...
      <div className="px-4 pb-4">
        <div className="flex gap-2 items-end">
          <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage() } }} placeholder={`Parlez à ${activeTemplate.name}...`} rows={1} className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#E50914] focus:outline-none resize-none" />

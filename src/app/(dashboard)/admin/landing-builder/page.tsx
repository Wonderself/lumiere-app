  ...
  if (!activePage) {
    return (
      <div className="space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A2E] font-[family-name:var(--font-playfair)]">Landing Builder</h1>
          <p className="text-sm text-gray-500 mt-1">Créez des pages promo pour vos films</p>
        </div>

        <div className="rounded-2xl border border-gray-200 bg-white p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <input value={pageName} onChange={e => setPageName(e.target.value)} placeholder="Nom de la page" className="rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#E50914] focus:outline-none" />
            <input value={filmTitle} onChange={e => setFilmTitle(e.target.value)} placeholder="Titre du film" className="rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#E50914] focus:outline-none" />
          </div>
        ...
        {pages.length === 0 ? (
          <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center">
            <Layout className="h-10 w-10 text-gray-500 mx-auto mb-3" />
            <p className="text-sm text-gray-500">Aucune landing page</p>
          </div>
        ) : (
          <div className="space-y-3">
            {pages.map(page => (
              <button key={page.id} onClick={() => setActivePage(page)} className="w-full text-left rounded-xl border border-gray-200 bg-white p-5 hover:border-gray-300 transition-colors flex items-center gap-4">
                <Layout className="h-5 w-5 text-[#E50914]" />
                <div className="flex-1">
                  <p className="text-sm font-medium text-[#1A1A2E]">{page.name}</p>
                  <p className="text-xs text-gray-500">{page.filmTitle} · {page.blocks.length} blocs · {page.status}</p>
                </div>
              </button>
            ))}
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <button onClick={() => setActivePage(null)} className="text-xs text-gray-500 hover:text-[#1A1A2E] mb-1">← Retour</button>
          <h1 className="text-xl font-bold text-[#1A1A2E]">{activePage.name}</h1>
          <p className="text-xs text-gray-500">{activePage.filmTitle} · {activePage.blocks.length} blocs</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setPreview(!preview)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${preview ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Block Palette */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase mb-3">Ajouter un bloc</p>
          <div className="space-y-2">
            {LANDING_BLOCKS.map(block => {
              const BIcon = BLOCK_ICONS[block.icon] || Layout
              return (
                <button key={block.type} onClick={() => addBlock(block.type)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-left transition-colors">
                  <BIcon className="h-4 w-4 text-[#E50914]" />
                  <div>
                    <p className="text-xs font-medium text-[#1A1A2E]">{block.label}</p>
        ...
        <div className="lg:col-span-3 space-y-4">
          {activePage.blocks.map((block, idx) => {
            ...
            if (preview) {
              return (
                <div key={block.id} className="rounded-xl border border-gray-200 bg-white p-8">
                  ...
                  {block.type === 'hero' && (
                    <div className="text-center py-8 bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl text-white">
                      ...
                      <p className="text-gray-500 mt-2">{block.content.tagline || 'Tagline'}</p>
                    </div>
                  )}
                  {block.type === 'synopsis' && <p className="text-sm text-gray-600 leading-relaxed">{block.content.text || 'Synopsis...'}</p>}
                  {block.type === 'cta' && (
                  ...
                  {!['hero', 'synopsis', 'cta'].includes(block.type) && <p className="text-sm text-gray-500 italic">[{blockDef?.label} — contenu configurable]</p>}
                </div>
              )
            }
            ...
            return (
              <div key={block.id} className="rounded-xl border border-gray-200 bg-white p-5">
                <div className="flex items-center gap-3 mb-4">
                  <BIcon className="h-4 w-4 text-[#E50914]" />
                  <span className="text-sm font-medium text-[#1A1A2E] flex-1">{blockDef?.label}</span>
                  <button onClick={() => moveBlock(block.id, 'up')} disabled={idx === 0} className="p-1 text-gray-500 hover:text-gray-600 disabled:opacity-30"><MoveUp className="h-4 w-4" /></button>
                  <button onClick={() => moveBlock(block.id, 'down')} disabled={idx === activePage.blocks.length - 1} className="p-1 text-gray-500 hover:text-gray-600 disabled:opacity-30"><MoveDown className="h-4 w-4" /></button>
                  <button onClick={() => removeBlock(block.id)} className="p-1 text-gray-500 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                </div>
                <div className="space-y-3">
                  {Object.entries(block.content).map(([key, value]) => (
                    <div key={key}>
                      <label className="text-[10px] text-gray-500 mb-1 block capitalize">{key.replace(/([A-Z])/g, ' $1')}</label>
                      {key === 'text' || key === 'tagline' ? (
                        <textarea value={value} onChange={e => updateBlockContent(block.id, key, e.target.value)} rows={2} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#E50914] focus:outline-none resize-none" />
                      ) : (
                        <input value={value} onChange={e => updateBlockContent(block.id, key, e.target.value)} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#E50914] focus:outline-none" />
                      )}
                    </div>
                  ))}
                </div>

  ...
  return (
    <div className="space-y-10">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Authentification 2 facteurs et journal d&apos;audit</p>
      </div>

      {/* 2FA Status */}
      <div className={`rounded-2xl border p-6 ${is2FAEnabled ? 'border-green-200 bg-green-50' : 'border-yellow-200 bg-yellow-50'}`}>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          ...
          {!totpSecret ? (
            ...
            <div className="space-y-5">
              {/* Secret */}
              <div>
                <label className="text-xs text-gray-500 mb-1.5 block">Secret (ajoutez dans votre app authenticator)</label>
                <div className="flex items-center gap-2">
                  <code className="flex-1 px-3 py-2 rounded-lg bg-gray-100 text-sm font-mono text-[#1A1A2E] select-all">
                    {showSecret ? totpSecret : '●'.repeat(20)}
                  </code>
                  <button onClick={() => setShowSecret(!showSecret)} className="p-2 rounded-lg hover:bg-gray-100">
                    {showSecret ? <EyeOff className="h-4 w-4 text-gray-500" /> : <Eye className="h-4 w-4 text-gray-500" />}
                  </button>
                  <button onClick={copySecret} className="p-2 rounded-lg hover:bg-gray-100">
              ...
              {/* Current Code */}
              <div>
                <label className="text-xs text-gray-500 mb-1.5 block">Code actuel ({timeLeft}s)</label>
                <div className="flex items-center gap-3">
              ...
              {/* Verify */}
              <div>
                <label className="text-xs text-gray-500 mb-1.5 block">Entrez le code pour vérifier</label>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="000000"
                    maxLength={6}
                    className="w-40 px-3 py-2 rounded-lg border border-gray-200 text-center text-lg font-mono tracking-widest focus:border-[#E50914] focus:outline-none"
                  />
                  <button
                    onClick={verifyTOTP}
      ...
      <div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
          <div className="divide-y divide-gray-100">
            {auditLogs.map(log => (
              <div key={log.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 px-4 sm:px-5 py-3 hover:bg-gray-50 transition-colors">
                <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                  <div className={`h-2.5 w-2.5 rounded-full shrink-0 ${
                    log.status === 'success' ? 'bg-green-500' : log.status === 'failure' ? 'bg-red-500' : 'bg-yellow-500'
                  }`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#1A1A2E]">{log.action}</p>

  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A2E] font-[family-name:var(--font-playfair)]">Veille Cinéma</h1>
          <p className="text-sm text-gray-500 mt-1">{RSS_SOURCES.length} sources · {MAJOR_FESTIVALS.length} festivals · Tendances industrie</p>
        </div>
        <button onClick={() => toast.success('Flux actualisé')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-gray-100 text-gray-600 hover:bg-gray-200 self-start sm:self-auto">
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          const TIcon = t.icon
          return <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium ${tab === t.key ? 'bg-[#E50914] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}><TIcon className="h-4 w-4" />{t.label}</button>
        })}
      </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-1 flex-wrap">
              ...
              {VEILLE_CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => setFilterCat(cat.id)} className={`px-3 py-1.5 rounded-lg text-xs flex items-center gap-1 ${filterCat === cat.id ? 'bg-[#E50914] text-white' : 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="space-y-3">
            {filteredFeed.map(item => {
              ...
              return (
                <div key={item.id} className="rounded-xl border border-gray-200 bg-white p-5 hover:shadow-sm transition-shadow">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] text-gray-500 flex items-center gap-0.5"><Clock className="h-3 w-3" />{item.date.toLocaleDateString('fr-FR')}</span>
                      </div>
                      <h3 className="text-sm font-semibold text-[#1A1A2E] mb-1">{item.title}</h3>
                      <p className="text-xs text-gray-500 line-clamp-2">{item.summary}</p>
                    </div>
                    <a href={item.url} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-gray-100 shrink-0">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {MAJOR_FESTIVALS.map(festival => (
              <div key={festival.name} className="rounded-xl border border-gray-200 bg-white p-5">
                <div className="flex items-center gap-2 mb-2">
                  <Award className="h-4 w-4 text-yellow-500" />
                  <h3 className="text-sm font-semibold text-[#1A1A2E] flex-1">{festival.name}</h3>
                  <span className={`text-[9px] px-2 py-0.5 rounded-full ${festival.prestige === 'A-list' ? 'bg-yellow-50 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}>{festival.prestige}</span>
                </div>
              ...
              <div key={cat.id}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                  {catSources.map(source => (
                    <div key={source.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-white">
                      <span className="text-lg">{source.icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-[#1A1A2E]">{source.name}</p>
                        <p className="text-[10px] text-gray-500 truncate">{source.url}</p>
                      </div>
                      <span className="text-[10px] text-gray-500">{source.language.toUpperCase()}</span>
                    </div>
                  ))}
                </div>
              </div>

...
export default async function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-10">
      {/* Header */}
      <div>
        <Link href="/admin/users/manage" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#1A1A2E] transition-colors mb-4">
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-[#1A1A2E]">{user.displayName || 'Sans nom'}</h1>
            <div className="flex items-center gap-3 mt-1">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{user.level}</span>
        ...
        {stats.map(s => {
          ...
          return (
            <div key={s.label} className="rounded-xl border border-gray-200 bg-white p-4 text-center">
              <SIcon className="h-4 w-4 text-gray-500 mx-auto mb-1" />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        ...
        {user.creditAccount ? (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div><p className="text-[10px] text-gray-500">Balance</p><p className="text-lg font-bold text-[#E50914]">{microToCredits(user.creditAccount.balance).toFixed(2)}</p></div>
            <div><p className="text-[10px] text-gray-500">Total acheté</p><p className="text-lg font-bold text-[#1A1A2E]">{microToCredits(user.creditAccount.totalPurchased).toFixed(2)}</p></div>
            <div><p className="text-[10px] text-gray-500">Total accordé</p><p className="text-lg font-bold text-[#1A1A2E]">{microToCredits(user.creditAccount.totalGranted).toFixed(2)}</p></div>
            <div><p className="text-[10px] text-gray-500">Total utilisé</p><p className="text-lg font-bold text-[#1A1A2E]">{microToCredits(user.creditAccount.totalUsed).toFixed(2)}</p></div>
            <div><p className="text-[10px] text-gray-500">Remboursé</p><p className="text-lg font-bold text-[#1A1A2E]">{microToCredits(user.creditAccount.totalRefunded).toFixed(2)}</p></div>
          </div>
        ) : (
          <p className="text-sm text-gray-500">Aucun compte crédit</p>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-sm font-semibold text-[#1A1A2E]">Transactions récentes</h2>
        </div>
        <div className="divide-y divide-gray-50">
          {recentTransactions.length === 0 ? (
            <div className="p-6 text-center text-sm text-gray-500">Aucune transaction</div>
          ) : recentTransactions.map(tx => (
            <div key={tx.id} className="flex items-center gap-4 px-6 py-3">
              <div className="flex-1">
                <p className="text-xs font-medium text-[#1A1A2E]">{tx.type.replace(/_/g, ' ')}</p>
                {tx.description && <p className="text-[10px] text-gray-500 truncate">{tx.description}</p>}
              </div>
              <p className={`text-xs font-semibold ${tx.amount > 0 ? 'text-green-500' : 'text-red-500'}`}>
                {tx.amount > 0 ? '+' : ''}{microToCredits(tx.amount).toFixed(2)}
              </p>
              <p className="text-[10px] text-gray-500">{new Date(tx.createdAt).toLocaleDateString('fr-FR')}</p>
            </div>
          ))}
        </div>
      </div>

...
const AUTONOMY_CONFIG = {
  low: { label: 'Bas', color: 'text-green-600', bg: 'bg-green-50' },
  medium: { label: 'Moyen', color: 'text-yellow-600', bg: 'bg-yellow-50' },
...
const STATUS_CONFIG = {
  ...
  disabled: { label: 'Désactivé', color: 'text-gray-500', dot: 'bg-gray-400' },
}
...
export default function CustomAgentsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-[#1A1A2E] font-[family-name:var(--font-playfair)]">Custom Agents Monitor</h1>
        <p className="text-sm text-gray-500 mt-1">Suivi des agents créés par les utilisateurs</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
          ...
          <p className="text-[10px] text-gray-500">En pause</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
          <p className="text-2xl font-bold text-red-600">{agents.filter(a => a.status === 'error').length}</p>
          <p className="text-[10px] text-gray-500">En erreur</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
          <p className="text-2xl font-bold text-gray-600">{agents.reduce((s, a) => s + a.executionCount, 0)}</p>
          <p className="text-[10px] text-gray-500">Total exécutions</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un agent..." className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-[#E50914] focus:outline-none" />
      </div>
            ...
            return (
              <div key={agent.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#1A1A2E]">{agent.name}</p>
                  <p className="text-[10px] text-gray-500">{agent.creator} · {agent.domain}</p>
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${auto.bg} ${auto.color} font-medium`}>
                  Autonomy: {auto.label}
                </span>
                <div className="text-right">
                  <p className="text-xs font-medium text-[#1A1A2E]">{agent.executionCount} exec</p>
                  <p className="text-[10px] text-gray-500">{agent.lastActive.toLocaleDateString('fr-FR')}</p>
                </div>
                <button onClick={() => toggleAgent(agent.id)} className="p-1.5 rounded-lg hover:bg-gray-100" title={agent.status === 'active' ? 'Désactiver' : 'Activer'}>

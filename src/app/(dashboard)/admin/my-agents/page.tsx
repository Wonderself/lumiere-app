...
export default function MyAgentsPage() {
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-100px)]">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200">
          <button onClick={() => { setChatAgent(null); setMessages([]) }} className="text-gray-500 hover:text-[#1A1A2E]"><ArrowLeft className="h-5 w-5" /></button>
          <AIcon className="h-5 w-5" style={{ color: chatAgent.color }} />
          <div>
            <p className="text-sm font-semibold text-[#1A1A2E]">{chatAgent.name}</p>
            <p className="text-[10px] text-gray-500">{TIER_CONFIG[chatAgent.tier].label}</p>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-3">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${m.role === 'user' ? 'bg-[#E50914] text-white' : 'bg-gray-100 text-gray-800'}`}>
                <p className="whitespace-pre-wrap">{m.content}</p>
              </div>
            </div>
          ))}
          {streaming && <div className="flex gap-1 px-4"><div className="h-2 w-2 rounded-full bg-gray-400 animate-bounce" /><div className="h-2 w-2 rounded-full bg-gray-400 animate-bounce" style={{ animationDelay: '150ms' }} /><div className="h-2 w-2 rounded-full bg-gray-400 animate-bounce" style={{ animationDelay: '300ms' }} /></div>}
        </div>
        <div className="px-4 pb-4 flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') sendMessage() }} placeholder={`Parlez à ${chatAgent.name}...`} className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-[#E50914] focus:outline-none" />
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-[#1A1A2E] font-[family-name:var(--font-playfair)]">My Agents</h1>
        <p className="text-sm text-gray-500 mt-1">{ALL_AGENTS.length} agents · Chat direct</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map(agent => {
          ...
          return (
            <button key={agent.slug} onClick={() => setChatAgent(agent)} className="text-left bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 hover:border-gray-300 hover:shadow-md transition-all">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-500">{TIER_CONFIG[agent.tier].label}</p>
                </div>
              </div>
